...
- Added a per-feature markdown conformance test suite (`Tests/MarkdownCoreTests`) written against CommonMark 0.31.2, covering headings, paragraphs and line breaks, thematic breaks, fenced code, block quotes, lists, task lists, code spans, emphasis, links, images, escapes, and tables. Expectations follow the specification rather than current behavior, so the suite documents what the renderer should do; 44 of its 92 cases fail today and each failure is filed under "Bug fixes" in `TODO.md`.
- Extracted the markdown engine into `MarkdownCore`, a local Swift package the app depends on, so it can be built and tested from the command line with no app host and no GUI session. `swift test --package-path MarkdownCore` runs the engine's tests in about a second, instead of building and launching the app under `xcodebuild test`. `MarkdownBlockParser`, `MarkdownHTMLBuilder`, `MarkdownSourceLineTable`, and `MarkdownSelectionRange` moved into the package and their API became `public`; the app files that use them now `import MarkdownCore`. The package is deliberately free of SwiftUI, UIKit, and AppKit — a UI-framework import there is what would push its tests back into an app host.
  ...
  - Split the package's tests into two targets so each can be run on its own: `MarkdownCoreTests` (parser and HTML builder, expected to pass) and `MarkdownCoreConformanceTests` (the CommonMark suite, expected to fail until the renderer catches up).
  - Added three test plans, all selectable from Xcode's Tests panel: the default `MarkdownPreview` plan runs the app tests plus `MarkdownCoreTests` (95 tests, passing), while `MarkdownCoreTests` and `MarkdownCoreConformanceTests` run each engine suite alone. The conformance suite is deliberately kept out of the default plan so an ordinary test run stays meaningful, with the known failures one plan-switch away.
