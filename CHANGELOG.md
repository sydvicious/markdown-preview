...
- Fixed paragraph spacing, which had collapsed so that a blank line between paragraphs produced no visible gap — the reason `SAMPLE.md` leaned on `&nbsp;` spacer lines to force space. Each top-level block is wrapped in its own `md-block` element, which makes every paragraph the sole (last) child of its wrapper and tripped a stylesheet rule, `p:last-child { margin-bottom: 0 }`, that was meant only to trim the trailing margin at the document's end. It instead zeroed every paragraph's bottom margin. The trim is now scoped to blocks that actually sit against a container edge (`.md-block:last-child > :last-child`, and the last child inside a quote or list item), so ordinary paragraphs keep their spacing while within-paragraph line breaks stay tight. `&nbsp;` spacer lines should no longer be needed.
